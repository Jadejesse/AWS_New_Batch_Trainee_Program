import React, { useEffect, useRef } from 'react';
import { Link } from 'react-router-dom';
import InfoPanel from '../../components/InfoPanel/InfoPanel';
import Avatar from '../../components/Avatar/Avatar';
import { useKonamiCode } from '../../hooks/useKonamiCode';
import FireworkRain from '../../components/Effects/FireworkRain';
import './Homepage.css';

const Homepage: React.FC = () => {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const rippleCanvasRef = useRef<HTMLCanvasElement>(null);
  const homepageBg = `${import.meta.env.BASE_URL}Homepage_image.png`;
  const isKonamiActive = useKonamiCode();

  // Water Ripple Effect
  useEffect(() => {
    const canvas = rippleCanvasRef.current;
    if (!canvas) return;
    const ctx = canvas.getContext('2d');
    if (!ctx) return;

    canvas.width = window.innerWidth;
    canvas.height = window.innerHeight;

    const width = canvas.width;
    const height = canvas.height;
    const size = width * height;
    let buffer1 = new Int32Array(size);
    let buffer2 = new Int32Array(size);
    const texture = ctx.getImageData(0, 0, width, height);

    // Load background image into texture for ripple
    const img = new Image();
    img.crossOrigin = "anonymous";
    img.src = homepageBg;
    img.onload = () => {
      const tempCanvas = document.createElement('canvas');
      tempCanvas.width = width;
      tempCanvas.height = height;
      const tempCtx = tempCanvas.getContext('2d', { willReadFrequently: true });
      if (tempCtx) {
        tempCtx.drawImage(img, 0, 0, width, height);
        const imgData = tempCtx.getImageData(0, 0, width, height);
        texture.data.set(imgData.data);
      }
    };

    const ripple = (x: number, y: number) => {
      const index = x + y * width;
      if (index >= 0 && index < size) {
        buffer1[index] = 512;
      }
    };

    const update = () => {
      const outData = ctx.createImageData(width, height);
      for (let i = width; i < size - width; i++) {
        buffer2[i] = ((buffer1[i - 1] + buffer1[i + 1] + buffer1[i - width] + buffer1[i + width]) >> 1) - buffer2[i];
        buffer2[i] -= buffer2[i] >> 5;

        const x = i % width;
        const y = Math.floor(i / width);

        let xOffset = (((buffer2[i - 1] - buffer2[i + 1])) >> 3);
        let yOffset = (((buffer2[i - width] - buffer2[i + width])) >> 3);

        if (xOffset !== 0 || yOffset !== 0) {
          const tx = Math.min(width - 1, Math.max(0, x + xOffset));
          const ty = Math.min(height - 1, Math.max(0, y + yOffset));
          const tIdx = (tx + ty * width) * 4;
          const oIdx = i * 4;

          outData.data[oIdx] = texture.data[tIdx];
          outData.data[oIdx + 1] = texture.data[tIdx + 1];
          outData.data[oIdx + 2] = texture.data[tIdx + 2];
          outData.data[oIdx + 3] = 255;
        } else {
          const oIdx = i * 4;
          outData.data[oIdx] = texture.data[oIdx];
          outData.data[oIdx + 1] = texture.data[oIdx + 1];
          outData.data[oIdx + 2] = texture.data[oIdx + 2];
          outData.data[oIdx + 3] = 255;
        }
      }
      [buffer1, buffer2] = [buffer2, buffer1];
      ctx.putImageData(outData, 0, 0);
    };

    const handleMouseMove = (e: MouseEvent) => {
      ripple(Math.floor(e.clientX), Math.floor(e.clientY));
    };

    let animationId: number;
    const loop = () => {
      update();
      animationId = requestAnimationFrame(loop);
    };
    loop();

    window.addEventListener('mousemove', handleMouseMove);
    return () => {
      window.removeEventListener('mousemove', handleMouseMove);
      cancelAnimationFrame(animationId);
    };
  }, [homepageBg]);

  // Trail Particles Effect
  useEffect(() => {
    const canvas = canvasRef.current;
    if (!canvas) return;

    const ctx = canvas.getContext('2d');
    if (!ctx) return;

    canvas.width = window.innerWidth;
    canvas.height = window.innerHeight;

    const particles: Array<{
      x: number;
      y: number;
      vx: number;
      vy: number;
      life: number;
      color: string;
    }> = [];

    const colors = [
      '#FF6B6B', '#4ECDC4', '#45B7D1', '#FFA07A',
      '#98D8C8', '#F7DC6F', '#BB8FCE', '#85C1E2'
    ];

    const handleMouseMove = (e: MouseEvent) => {
      for (let i = 0; i < 3; i++) {
        particles.push({
          x: e.clientX,
          y: e.clientY,
          vx: (Math.random() - 0.5) * 2,
          vy: (Math.random() - 0.5) * 2,
          life: 1,
          color: colors[Math.floor(Math.random() * colors.length)]
        });
      }
    };

    const animate = () => {
      ctx.clearRect(0, 0, canvas.width, canvas.height);

      for (let i = particles.length - 1; i >= 0; i--) {
        const p = particles[i];
        p.x += p.vx;
        p.y += p.vy;
        p.life -= 0.01;

        if (p.life <= 0) {
          particles.splice(i, 1);
          continue;
        }

        ctx.beginPath();
        ctx.arc(p.x, p.y, 3, 0, Math.PI * 2);
        ctx.fillStyle = p.color;
        ctx.globalAlpha = p.life;
        ctx.fill();

        // Draw trails
        if (i > 0) {
          const prev = particles[i - 1];
          ctx.beginPath();
          ctx.moveTo(p.x, p.y);
          ctx.lineTo(prev.x, prev.y);
          ctx.strokeStyle = p.color;
          ctx.lineWidth = 2;
          ctx.globalAlpha = p.life * 0.5;
          ctx.stroke();
        }
      }

      ctx.globalAlpha = 1;
      requestAnimationFrame(animate);
    };

    window.addEventListener('mousemove', handleMouseMove);
    animate();

    const handleResize = () => {
      canvas.width = window.innerWidth;
      canvas.height = window.innerHeight;
    };
    window.addEventListener('resize', handleResize);

    return () => {
      window.removeEventListener('mousemove', handleMouseMove);
      window.removeEventListener('resize', handleResize);
    };
  }, []);

  return (
    <div className="homepage" style={{ ['--homepage-bg' as any]: `url('${homepageBg}')` }}>
      <canvas ref={rippleCanvasRef} className="homepage-ripple-canvas" style={{ position: 'fixed', top: 0, left: 0, zIndex: 0 }} />
      <canvas ref={canvasRef} className="homepage-particle-canvas" />
      {isKonamiActive && <FireworkRain />}
      <Avatar />
      <div className="sidebar">
        <InfoPanel />
      </div>
      <div className="main-content">
        {/* Main content area - background image visible here */}
      </div>
      <div className="right-sidebar">
        <div className="sidebar-tab">
          <span>📚</span>
          <span className="tab-text">Resources</span>
        </div>
        <div className="right-panel">
          <h3 className="panel-title">Learning Resources</h3>
          <div className="right-nav-buttons">
            <Link to="/aws-services" className="right-nav-button">
              What are AWS Services
            </Link>
            <Link to="/aws-customers" className="right-nav-button">
              Who Uses AWS
            </Link>
            <Link to="/aws-infrastructure" className="right-nav-button">
              AWS Infrastructure (Region/AZ/DC)
            </Link>
            <Link to="/dco-importance" className="right-nav-button">
              Why DCO Job is Important
            </Link>
          </div>
        </div>
      </div>
      <div className="footer-contacts">
        <div className="contact-icons">
          <a href="https://linkedin.com" target="_blank" rel="noopener noreferrer" className="contact-icon">
            <svg viewBox="0 0 24 24" fill="currentColor">
              <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z" />
            </svg>
          </a>
          <a href="https://github.com" target="_blank" rel="noopener noreferrer" className="contact-icon">
            <svg viewBox="0 0 24 24" fill="currentColor">
              <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" />
            </svg>
          </a>
          <a href="mailto:chosen1edwin@gmail.com" className="contact-icon">
            <svg viewBox="0 0 24 24" fill="currentColor">
              <path d="M0 3v18h24v-18h-24zm6.623 7.929l-4.623 5.712v-9.458l4.623 3.746zm-4.141-5.929h19.035l-9.517 7.713-9.518-7.713zm5.694 7.188l3.824 3.099 3.83-3.104 5.612 6.817h-18.779l5.513-6.812zm9.208-1.264l4.616-3.741v9.348l-4.616-5.607z" />
            </svg>
          </a>
          <a href="https://youtube.com/@jadelee666yu" target="_blank" rel="noopener noreferrer" className="contact-icon">
            <svg viewBox="0 0 24 24" fill="currentColor">
              <path d="M19.615 3.184c-3.604-.246-11.631-.245-15.23 0-3.897.266-4.356 2.62-4.385 8.816.029 6.185.484 8.549 4.385 8.816 3.6.245 11.626.246 15.23 0 3.897-.266 4.356-2.62 4.385-8.816-.029-6.185-.484-8.549-4.385-8.816zm-10.615 12.816v-8l8 3.993-8 4.007z" />
            </svg>
          </a>
        </div>
      </div>
    </div>
  );
};

export default Homepage;
